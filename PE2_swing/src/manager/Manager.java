...
public class Manager{
    ...
    public int runGame(){
        ...
        for (int i =0 ;i<playerCount ; i++){
            players.add(new Player(i,pieceCount,ui));
        }
        ...
        while(true){
            boolean finished = playTurn(players.get(currentPlayer));        //todo 
            if (finished){
    ...
    public boolean playTurn(Player player){
		...
		//한명의 턴을 진행
        while (!allZero(player.permittedMoves)|| player.permittedThrows !=0 ){
            while(player.permittedThrows != 0){
                ...
                player.thrownYut(yut);
                
            }  
            
            //move piece as given amount
            //말 움직일떄 stack되어있는 말도 전부 움직이게 바꿔야함
            int piece_id = ui.choicePiece(currentPlayer);					//todo : why is this the one that asks the view? cause he is the one calling the view to update
            int chosen_yut;
            while(player.permittedMoves[chosen_yut = ui.choiceYut()] == 0){}
            if (player.movePiece(piece_id, chosen_yut, board)) {
                return true;
            }
	    }
        return false;
